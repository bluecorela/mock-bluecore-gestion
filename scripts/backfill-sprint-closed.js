...
async function main() {
  ...
  const expectedMembers = personnelSnapshot.docs
    .map((d) => d.data())
    .filter((p) => {
      ...
      // Excluir arquitecto
      if (role === "arquitecto") return false;
  ...
  console.log(`Integrantes esperados (personnel): ${expectedMembers}`);
 
  // 3) Listar sprints
  const sprintsSnap = await db.collection(`teams/${TEAM_ID}/sprints`).get();
  ...
  for (const s of sprints) {
    ...
    console.log(
      `[${s.id}] evaluatedMembers=${evaluatedMembers}, esperados=${expectedMembers}, completo=${isComplete}`
    );
  }
