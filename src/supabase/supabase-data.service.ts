...
interface SprintMetricEmployeeRow {
  id: string;
  employee_code: string | null;
  full_name: string;
  email: string | null;
...
interface SprintMetricRow {
  ...
  component_1_score: number | null;
  component_2_score: number | null;
  component_3_score: number | null;
...
interface PerformanceEvaluationRow {
  id: string;
  employee_id: string;
  evaluator_id: string;
...
export class SupabaseDataService {
  ...
  async getPersonnelById(id: string) {
    try {
      ...
      if (String(error?.message ?? '').startsWith('Employee not found:'))
        return null;
      throw error;
    }
  }
  ...
  private async getV2Personnel(employeeIds?: string[]): Promise<Personnel[]> {
    ...
    const [roleAssignments, memberships, absences] = await Promise.all([
      database
        .from('employee_roles')
        .select('employee_id,role_id,is_primary,started_at')
        .in('employee_id', ids)
        .is('ended_at', null)
        .order('is_primary', { ascending: false }),
      database
        .from('team_memberships')
        .select('employee_id,team_id,started_at')
        .in('employee_id', ids)
        .eq('is_active', true)
        .order('started_at', { ascending: false }),
      database
        .from('employee_absences')
        .select('employee_id,replacement_employee_id')
        .in('employee_id', ids)
        .eq('status', 'active'),
    ]);
    ...
    const roleIds = [
      ...new Set((roleAssignments.data ?? []).map((row) => row.role_id)),
    ];
    const teamIds = [
      ...new Set((memberships.data ?? []).map((row) => row.team_id)),
    ];
    const [roles, teams] = await Promise.all([
      roleIds.length
        ? database.from('roles').select('id,code,name').in('id', roleIds)
    ...
    const roleRows = (roles.data ?? []) as RoleSummaryRow[];
    const teamRows = (teams.data ?? []) as TeamSummaryRow[];
    const employeeRoleAssignments = (roleAssignments.data ??
      []) as EmployeeRoleAssignmentRow[];
    const teamMemberships = (memberships.data ?? []) as TeamMembershipRow[];
    const rolesById = new Map(roleRows.map((role) => [role.id, role] as const));
    const teamsById = new Map(teamRows.map((team) => [team.id, team] as const));
    const primaryRoleByEmployee = new Map<string, RoleSummaryRow | undefined>();
    for (const assignment of employeeRoleAssignments) {
      if (!primaryRoleByEmployee.has(assignment.employee_id)) {
        primaryRoleByEmployee.set(
    ...
    const vacationingIds = new Set(
      (absences.data ?? []).map((absence) => absence.employee_id),
    );
    const legacyRoles: Record<string, string> = {
      ...
      QA_ENGINEER: 'Ingeniero de QA',
      WELLBEING_CREATOR: 'Creador de Bienestar',
      INTERN: 'Pasante',
    };
  ...
  async getVacationingPersonnel(): Promise<Personnel[]> {
    const database = this.supabaseClient.getV2Client();
    const { data: absences, error } = await database
      .from('employee_absences')
    ...
    const employeeIds = [
      ...new Set(absences.map((absence) => absence.employee_id)),
    ];
    const teamIds = [
      ...new Set(absences.map((absence) => absence.team_id).filter(Boolean)),
    ];
    const [
  ...
  async createPersonnel(data: CreatePersonnelData): Promise<{ id: string }> {
    ...
    const { data: id, error } = await this.supabaseClient
      .getV2Client()
  ...
  async updatePersonnel(personnelId: string, data: UpdatePersonnelData) {
    ...
    if (data.status !== undefined)
      payload.status = data.status === 'inactivo' ? 'inactive' : 'active';
    if (data.teamId !== undefined) payload.teamCode = data.teamId;
    if (data.createdBy !== undefined) payload.createdBy = data.createdBy;
    if (data.role !== undefined) {
      const roleCode = roleCodes[data.role];
      if (!roleCode)
        throw new Error(`Unsupported personnel role: ${data.role}`);
      payload.roleCode = roleCode;
    }
    const { data: employeeId, error } = await this.supabaseClient
      .getV2Client()
  ...
  async getSprintsByTeam(teamId: string): Promise<Sprint[]> {
    ...
    if (error) throw error;
    return ((data ?? []) as SprintRow[]).map((sprint) =>
      this.mapV2Sprint(sprint, team.code),
    );
  }
  ...
  async getSprint(teamId: string, sprintId: string): Promise<Sprint | null> {
    ...
    if (error) throw error;
    return data ? this.mapV2Sprint(data as SprintRow, team.code) : null;
  }
  ...
  async getMembersBySprint(
    ...
    if (!sprint) return [];
    const { data: metrics, error } = await database
      .from('sprint_member_metrics')
      .select('*')
    ...
    const employeeIds = [
      ...new Set(
        metrics
          .flatMap((metric) => [metric.employee_id, metric.evaluated_by])
          .filter(Boolean),
      ),
    ];
    const { data: employees, error: employeeError } = await database
      ...
      .in('id', employeeIds);
    if (employeeError) throw employeeError;
    const employeeRows = (employees ?? []) as SprintMetricEmployeeRow[];
    const metricRows = metrics as SprintMetricRow[];
    const employeesById = new Map(
      employeeRows.map((employee) => [employee.id, employee]),
    );
    return metricRows.map((metric): SprintMember => {
      ...
      return {
        ...
        employeeCode:
          ...
          this.slug(employee?.full_name ?? metric.employee_id),
        sprint_id: sprint.id,
  ...
  private mapV2Sprint(row: SprintRow, teamCode: string): Sprint {
    return {
      id: row.id,
  ...
  async getRotationHistory(): Promise<RotationHistory[]> {
    const database = this.supabaseClient.getV2Client();
    const { data: events, error } = await database
      .from('team_rotation_events')
      .select('*')
    ...
    const teamIds = [
      ...new Set(
        events
          .flatMap((event) => [event.from_team_id, event.to_team_id])
          .filter(Boolean),
      ),
    ];
    const [
    ...
    const legacyTypes: Record<string, string> = {
      rotation: 'rotacion',
      vacation_start: 'vacaciones',
      vacation_end: 'reintegracion',
      assignment: 'cubriendo-vacaciones',
      unassignment: 'fin-cobertura-vacaciones',
    };
    return events.map((event) => {
      ...
      return {
        ...
        personnelId:
          employee?.employee_code ?? employee?.id ?? event.employee_id,
      };
    });
  }
  ...
  async manageEmployeeMovement(input: {
    ...
    const sourceTeam = input.sourceTeamId
      ? await this.resolveV2Team(input.sourceTeamId)
      : null;
    const destinationTeam = input.destinationTeamId
      ? await this.resolveV2Team(input.destinationTeamId)
      : null;
    const replacement = input.replacementId
      ? await this.resolveV2Employee(input.replacementId)
      : null;
    const { data: movementId, error } = await database.rpc(
  ...
  async savePerformanceConfig(data: Record<string, unknown>) {
    return this.saveEvaluationConfig(
      'performance',
  ...
  private async getPublishedEvaluationConfig(
  ...
  private async saveEvaluationConfig(
    ...
    if (
      latest &&
      JSON.stringify(latest.configuration) === JSON.stringify(configuration)
    ) {
  ...
  async savePerformanceEvaluation(data: CreatePerformanceEvaluationDto) {
    ...
    const { data: cycle, error: cycleError } = await database
      .from('performance_cycles')
      .select('id')
      .eq('team_id', team.id)
      .in('status', ['enabled', 'in_progress'])
    ...
    if (!cycle)
      throw new Error(
        `No active performance cycle was found for team ${data.teamId}`,
      );

    const templateVersionId =
      await this.getLatestTemplateVersion('performance');
    ...
    const { data: evaluationId, error } = await database.rpc(
  ...
  async getPerformanceHistory(teamId: string) {
    const database = this.supabaseClient.getV2Client();
    const team = await this.resolveV2Team(teamId);
    const { data: evaluations, error } = await database
      .from('performance_evaluations')
      .select('*')
      .eq('team_id', team.id)
    ...
    const employeeIds = [
      ...new Set(
        evaluations.flatMap((row) => [row.employee_id, row.evaluator_id]),
      ),
    ];
    ...
    const names = new Map(
      (employees ?? []).map((row) => [row.id, row.full_name]),
    );
    const answersByEvaluation = new Map<
      string,
      Record<string, { score: number; comment: string }>
    >();
    const answerRows = (answers ?? []) as PerformanceAnswerRow[];
    const evaluationRows = evaluations as PerformanceEvaluationRow[];
    for (const answer of answerRows) {
      const current = answersByEvaluation.get(answer.evaluation_id) ?? {};
      current[answer.question_key] = {
  ...
  async saveOtoEvaluation(data: CreateOtoEvaluationDto) {
    ...
    const sprintNumbers = [...data.period.matchAll(/Sprint\s+(\d+)/gi)].map(
    ...
    if (sprintNumbers.length >= 2) {
      ...
      const sprintByNumber = new Map(
        (sprints ?? []).map((sprint) => [sprint.sprint_number, sprint.id]),
      );
      ...
      if (!sprintFromId || !sprintToId) {
        throw new Error(
          `Sprint range not found for team ${data.teamId}: ${data.period}`,
        );
      }
    }
  ...
  async getOtoHistory(teamId: string) {
    ...
    const employeeIds = [
      ...new Set(
        sessions.flatMap((row) => [row.employee_id, row.evaluator_id]),
      ),
    ];
    const { data: employees, error: employeeError } = await database
      .from('employees')
  ...
  private async getLatestTemplateVersion(type: 'performance' | 'one_to_one') {
    ...
    if (templateError) throw templateError;
    if (!template)
      throw new Error(`Active evaluation template not found: ${type}`);
    const { data: version, error: versionError } = await database
      .from('evaluation_template_versions')
      .select('id')
      .eq('template_id', template.id)
    ...
    if (!version)
      throw new Error(
        `Published evaluation template version not found: ${type}`,
      );
    return version.id;
  }
  ...
  async enablePerformance(teamId: string, adminName: string) {
    ...
    const [
      ...
      { data: excludedRoles, error: roleError },
    ] = await Promise.all([
      database
        .from('team_memberships')
    ...
    const excludedRoleIds = new Set(
      (excludedRoles ?? []).map((role) => role.id),
    );
    const expectedEmployees = new Set(
      (memberships ?? [])
        .filter((membership) => !excludedRoleIds.has(membership.role_id))
        .map((membership) => membership.employee_id),
    );
    const enabledAt = new Date().toISOString();
    const { data: cycle, error } = await database
      .from('performance_cycles')
      .insert({
        team_id: team.id,
  ...
  async getPerformanceEnablements(teamId?: string) {
    ...
    const adminIds = [
      ...new Set(cycles.map((cycle) => cycle.enabled_by).filter(Boolean)),
    ];
    const [
      { data: teams, error: teamError },
  ...
  async getSprintEvaluationStatus(teamId: string, specificSprintId?: string) {
    ...
    const activeSprint = specificSprintId
      ? sprints.find((s) => s.code === specificSprintId)
      : sprints
          .filter((s) => s.sprint_closed !== true)
          .sort(
            (a, b) =>
              this.getSprintNumero(a.code) - this.getSprintNumero(b.code),
          )[0];

    const maxSprintNum = sprints.reduce(
      (max, sprint) => Math.max(max, this.getSprintNumero(sprint.code)),
      0,
    );
    const sprintNumber = activeSprint
      ? this.getSprintNumero(activeSprint.code)
      : maxSprintNum + 1;
    const sprintId = activeSprint
      ? activeSprint.code
      : `sprint-${sprintNumber}`;
    const rawEvaluatedMembers = await this.getMembersBySprint(teamId, sprintId);
    const evaluatedNames = rawEvaluatedMembers.map((e) =>
      String(e.name || '')
        .toLowerCase()
        .trim(),
    );
    ...
    if (activeSprint) {
      dates.startDate = activeSprint.start_date
        ? activeSprint.start_date.split('T')[0]
        : '';
      dates.endDate = activeSprint.end_date
