...
export class SprintItemsService {
  ...
  async create(
    ...
    if (kind === 'initiatives') {
      return this.repository.createInitiative(teamId, sprintId, payload);
    }
    if (kind === 'stories')
  ...
  async update(
    ...
    const sprint = await this.requireWritableSprint(teamId, sprintId);
    if (kind === 'stories') {
      const currentItems = await this.repository.findAll(
        tables.stories,
        sprintId,
      );
    ...
    if (kind === 'initiatives') {
      const row = await this.repository.updateInitiative(
        teamId,
        sprintId,
        itemId,
        payload,
    ...
    if (
      (kind === 'bugs' &&
        ['open', 'in_progress'].includes(String(payload.status))) ||
      (kind === 'risks' &&
        ['open', 'at_risk', 'monitoring'].includes(String(payload.status)))
    )
  ...
  async moveStory(
    ...
    const source = await this.requireSprint(teamId, sourceSprintId);
    const target = await this.requireSprint(teamId, targetSprintId);
