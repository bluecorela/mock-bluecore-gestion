...
import {
  ...
  UpdateSidebarModuleDto,
} from './dto/sidebar-module.dto';
...
export class SidebarModulesController {
  ...
  @ApiOperation({
    summary: 'Listar configuración completa del sidebar (solo Admin)',
  })
  getConfiguration() {
  ...
  @UseGuards(AuthGuard, AdminGuard)
  @ApiOperation({
    summary: 'Crear un módulo del sidebar y asignarlo a roles (solo Admin)',
  })
  ...
  @UseGuards(AuthGuard, AdminGuard)
  @ApiOperation({
    summary: 'Editar un módulo del sidebar y sus roles (solo Admin)',
  })
  updateModule(
    @Param('moduleId') moduleId: string,
    @Body() body: UpdateSidebarModuleDto,
  ) {
  ...
  async getModulesByRole(
    ...
    if (!modules || modules.length === 0) {
      throw new NotFoundException(`No existen módulos para el rol ${role}`);
    }
